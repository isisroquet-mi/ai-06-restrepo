...
- Paper: *The Race Between Man and Machine: Implications of Technology for Growth, Factor Shares and Employment*
...
|---|---|---|---|---|
| Proposition 3, technology-constrained automation with `dN = 0`: wage decomposition and sign condition | `propositionThreeWageEffectProof` | proved, partial paper coverage | `ProofInterface.lean` | The two comparative-static equations are premises; the production model, differentiation, threshold-capital result, and remainder of the paper are not formalized. |

## Intake Checklist

- [x] Pin the exact source bytes and version locally (`source.pdf`, ignored by Git).
- [ ] Complete the source-only selected-presentation and material-atom inventory.
- [x] Complete the outside-Lean formula/dependency sanity pass and working memo for the selected algebraic scope.
...
- [x] Create actual source definitions/models and one transparent Spec per
      selected claim in `PaperInterface.lean`.
- [x] Create each distinct proof endpoint in `ProofInterface.lean`.
